...
function TradeInContent() {
  ...
  useEffect(() => {
    ...
    const timeout = globalThis.setTimeout(() => {
      try {
        ...
        if (!hasData) return
        const payload = {
          step, lookupMethod, selectedYear, selectedMake, selectedModel, selectedTrim,
          mileage, condition, hasAccident, hasMechanicalIssues, hasLien, payoffAmount,
          additionalNotes, email, phone, postalCode, vinNumber,
  ...
  const handlePhotoUpload = (angle: string, file: File | null) => {
    ...
    reader.onloadend = () => {
      setPhotos(prev => ({ ...prev, [angle]: { file, preview: reader.result as string } }))
    }
  ...
  const handlePlateLookup = async () => {
    ...
    setFoundVehicle({ year: 2021, make: "Honda", model: "Accord", trim: "Sport 2.0T", vin: "1HGCV2F34MA012345", estimatedMileage: "45,000", color: "Crystal Black Pearl" })
  ...
  const handleVinLookup = async () => {
    ...
    try {
      ...
      if (data.success && data.vehicle) {
        ...
        setVehicleFound(true)
        setFoundVehicle({ year: Number.parseInt(v.year) || 0, make: v.make, model: v.model, trim: v.trim, vin: v.vin, estimatedMileage: "", color: "" })
  ...
  const nextStep = () => {
    ...
    setTimeout(() => { stepContentRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }, 100)
  }
            ...
            <div className="max-w-xl mx-auto">
              <Card className="shadow-2xl border-0 bg-white/[0.03] backdrop-blur-xl border border-white/10">
                <CardContent className="p-6 sm:p-8">
                  <Tabs value={lookupMethod} onValueChange={(v: string) => setLookupMethod(v as "vin" | "plate" | "manual")}>
                      ...
                      <div className="space-y-4 pt-4 border-t">
                        ...
                        <div className="flex items-start space-x-3 p-4 border rounded-lg">
                          ...
                          <div className="space-y-1">
                            <Label htmlFor="accident" className="cursor-pointer font-semibold">Has this vehicle been in an accident?</Label>
                            <p className="text-sm text-muted-foreground">Include any collision, even minor fender benders</p>
                          </div>
                        </div>
                        <div className="flex items-start space-x-3 p-4 border rounded-lg">
                          <Checkbox id="mechanical" checked={hasMechanicalIssues} onCheckedChange={(c) => setHasMechanicalIssues(c as boolean)} />
                          <div className="space-y-1">
                            <Label htmlFor="mechanical" className="cursor-pointer font-semibold">Are there any mechanical issues?</Label>
                            <p className="text-sm text-muted-foreground">Check engine light, transmission issues, unusual sounds, etc.</p>
                          </div>
                        </div>
                        <div className="flex items-start space-x-3 p-4 border rounded-lg">
                          <Checkbox id="lien" checked={hasLien} onCheckedChange={(c) => setHasLien(c as boolean)} />
                          <div className="space-y-1">
                            <Label htmlFor="lien" className="cursor-pointer font-semibold">Is there a loan or lien on this vehicle?</Label>
                            <p className="text-sm text-muted-foreground">We can pay off your lender directly</p>
                          </div>
                        </div>
                        {hasLien && (
                  ...
                  <Card className="shadow-lg">
                    ...
                    <CardContent className="space-y-6">
                        ...
                        {["Front", "Back", "Interior", "Dashboard"].map((angle) => (
                          <div key={angle} className="relative">
                            <input
                              ...
                              onChange={(e) => { handlePhotoUpload(angle, e.target.files?.[0] || null); e.currentTarget.value = "" }}
                            />
                            {photos[angle] ? (
                              <div className="aspect-video rounded-lg overflow-hidden relative group/photo border-2 border-green-500">
                                ...
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/photo:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                  ...
                                  <button type="button" aria-label={`Remove ${angle} photo`} onClick={() => removePhoto(angle)} className="bg-red-500 text-white px-2 py-1 rounded text-xs font-semibold relative z-10">Remove</button>
                                </div>
                      ...
                      {Object.keys(photos).length > 0 && (
                        <p className="text-sm text-green-600 font-semibold">✓ {Object.keys(photos).length} photo{Object.keys(photos).length > 1 ? 's' : ''} added</p>
                      )}
                      <div className="p-4 bg-teal-50 dark:bg-teal-950/20 border border-teal-200 dark:border-teal-800 rounded-lg">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-teal-700 dark:text-teal-300">Vehicles with photos typically receive offers 5-10% higher than those without.</p>
                          </div>
                        </div>
                      </div>
                      <div className="flex gap-3 pt-4">
                      ...
                      <div className="p-4 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-green-700 dark:text-green-300">We never share your information. No spam calls, guaranteed.</p>
                          </div>
                        </div>
                      </div>
                      <div className="flex gap-3 pt-4">
                        ...
                        <Button
                          ...
                          disabled={!email || !phone || !postalCode || !isValidEmail(email) || !isValidCanadianPhoneNumber(phone) || !isValidCanadianPostalCode(postalCode)}
                        >
                  ...
                  <CardContent className="p-6 space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-primary/50 via-primary to-primary/50 rounded-full" style={{ width: '100%' }} />
                      </div>
                    </div>
                    {offer.payoff > 0 && (
                      <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg space-y-2">
                        <div className="flex justify-between"><span>Trade-In Offer</span><span className="font-semibold">${offer.offerAmount.toLocaleString()}</span></div>
                        <div className="flex justify-between text-red-600"><span>Loan Payoff</span><span className="font-semibold">-${offer.payoff.toLocaleString()}</span></div>
                        <div className="border-t pt-2 flex justify-between font-bold text-lg">
                          <span>Your Equity</span>
                          <span className={offer.equity >= 0 ? "text-green-600" : "text-red-600"}>${Math.abs(offer.equity).toLocaleString()}{offer.equity < 0 && " owed"}</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-3 gap-2 sm:gap-4 text-center text-xs sm:text-sm">
                      <div><Zap className="h-6 w-6 mx-auto mb-1 text-primary" /><p className="font-semibold">Instant Offer</p><p className="text-xs text-muted-foreground">No waiting</p></div>
                      <div><Truck className="h-6 w-6 mx-auto mb-1 text-primary" /><p className="font-semibold">Free Pickup</p><p className="text-xs text-muted-foreground">Canada-wide</p></div>
                      <div><CreditCard className="h-6 w-6 mx-auto mb-1 text-primary" /><p className="font-semibold">24h Payment</p><p className="text-xs text-muted-foreground">E-Transfer or cheque</p></div>
                    </div>
                    ...
                    <p className="text-center text-sm text-muted-foreground">Questions? Call us at <strong>{PHONE_TOLL_FREE}</strong></p>
                  </CardContent>
                </Card>

                <Card className="shadow-lg">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2"><CheckCircle className="h-5 w-5 text-green-600" />What Happens Next?</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {[
                        { n: 1, title: "Accept Your Offer", desc: "Click accept and confirm your details" },
                        { n: 2, title: "Schedule Pickup", desc: "We come to you, anywhere in Canada" },
                        { n: 3, title: "Get Paid", desc: "Payment within 24 hours via e-Transfer" },
                      ].map((item) => (
                        <div key={item.n} className="flex gap-4">
                          <div className="flex-shrink-0 w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center font-bold text-primary">{item.n}</div>
                          <div><h4 className="font-semibold">{item.title}</h4><p className="text-sm text-muted-foreground">{item.desc}</p></div>
                        </div>
                      ))}
                    </div>
                  </CardContent>
          ...
          <section className="py-20 bg-background">
            <div className="container mx-auto px-4">
              <div className="text-center mb-16">
                ...
                <h2 className="text-3xl md:text-4xl font-bold mb-4">The Smarter Way to Trade-In</h2>
                <p className="text-xl text-muted-foreground max-w-2xl mx-auto">Skip the dealership games. Get a fair price in 60 seconds, not 6 hours.</p>
              </div>
        ...
        {!showOffer && (
          <section className="py-20 bg-primary text-primary-foreground">
            <div className="container mx-auto px-4 text-center">
              ...
              <Button size="lg" variant="secondary" className="h-14 px-8 text-lg" onClick={() => globalThis.scrollTo({ top: 0, behavior: 'smooth' })}>
                Get My Instant Offer <ArrowRight className="ml-2 h-5 w-5" />
              </Button>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              ...
              <ol className="text-sm text-green-700 space-y-2">
                <li className="flex items-start gap-2"><span className="bg-green-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">1</span><span>We&apos;ll contact you within 2 hours to schedule a pickup</span></li>
                <li className="flex items-start gap-2"><span className="bg-green-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">2</span><span>Free pickup anywhere in Canada at your convenience</span></li>
                <li className="flex items-start gap-2"><span className="bg-green-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">3</span><span>Get paid within 24 hours via e-Transfer or cheque</span></li>
              </ol>
            </div>
            <div className="space-y-3">
              <div>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  for (const [angle, photo] of Object.entries(photos)) {
                    if (photo?.preview) photoData[angle] = photo.preview
                  }
                  const response = await fetch('/api/v1/trade-in/accept', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      quoteId: offer?.quoteId, vehicleYear: selectedYear, vehicleMake: selectedMake,
                      vehicleModel: selectedModel, mileage, condition: condition, postalCode,
                      photos: Object.keys(photoData).length > 0 ? photoData : undefined,
                      offerAmount: offer?.offerAmount, customerEmail: email, customerPhone: phone,
                    })
                  })
                  const data = await response.json()
                  if (data.success) {
        ...
        <DialogContent className="sm:max-w-lg">
                    ...
                    try {
                      if (user) {
                        await fetch('/api/v1/trade-in/save', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ quoteId: offer?.quoteId, vehicleYear: selectedYear, vehicleMake: selectedMake, vehicleModel: selectedModel, mileage, condition: condition, postalCode, offerAmount: offer?.offerAmount, customerEmail: email || user.email, customerPhone: phone })
                        })
                      }
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button onClick={async () => {
              ...
              } catch (error) { console.error('Error saving trade-in:', error) }
              try { globalThis.localStorage.removeItem(TRADE_IN_DRAFT_KEY) } catch { /* localStorage unavailable */ }
              setShowApplyModal(false)
              const params = new URLSearchParams({ tradeIn: String(offer?.offerAmount || 0), quoteId: offer?.quoteId || '', tradeInVehicle: encodeURIComponent(offer?.vehicle || '') })
              globalThis.location.href = `/inventory?${params.toString()}`
            }}>
              <ArrowRight className="mr-2 h-4 w-4" />Browse Inventory
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
