...
async function fetchDeliveryQuote(request: NextRequest, postalCode: string):
  Promise<{ ok: true; distanceKm: number; deliveryCost: number } | { ok: false; res: Response }> {
  const quoteUrl = new URL(`/api/v1/deliveries/quote?postalCode=${encodeURIComponent(postalCode)}`, request.url)
  ...
  if (!quoteResponse.ok) {
    ...
    return { ok: false, res: apiError(ErrorCode.VALIDATION_ERROR, quoteError?.error || "Unable to calculate delivery quote", 400) }
  }
...
function handleExistingDelivery(
  ...
  if (isSameSchedule) {
    return apiSuccess(toDeliveryDto(existing, orderRef, { idempotentReplay: true }))
  }
  return apiError(ErrorCode.VALIDATION_ERROR, "An active delivery already exists for this order", 409)
}

function buildDeliveryNotes(postal: string, specialInstructions: unknown): string | null {
  const notes = [
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!order) return apiError(ErrorCode.NOT_FOUND, "Order not found", 404)
    ...
    const { data: insertedRows, error: insertError } = await supabase
      ...
      .insert({
        ...
        delivery_notes: buildDeliveryNotes(normalizedPostal, specialInstructions),
      })
      .select("id, status, scheduled_date, scheduled_time_slot, estimated_delivery_date, distance_km, delivery_fee, created_at")
      .single()
    ...
    return apiSuccess(toDeliveryDto(insertedRows as DeliveryRow, orderRef))
  } catch (error) {
