...
import { FIXTURE_SUMMARY } from "@/__tests__/fixtures/carfax"
...
describe("getCachedSummary", () => {
  it("returns ok(payload) on a found row", async () => {
    const c = clientWithSelect({
      data: { vin: FIXTURE_SUMMARY.vin, payload: FIXTURE_SUMMARY },
      error: null,
    })
    const r = await getCachedSummary(FIXTURE_SUMMARY.vin, () => c as never)
    expect(r.ok).toBe(true)
    if (r.ok) expect(r.value?.vin).toBe(FIXTURE_SUMMARY.vin)
  })

  it("returns ok(null) when no row exists", async () => {
  ...
  it("catches thrown exceptions into err", async () => {
    ...
    const r = await getCachedSummary(FIXTURE_SUMMARY.vin, factory as never)
    expect(r.ok).toBe(false)
...
describe("upsertSummary", () => {
  it("calls upsert with vin onConflict and returns ok(summary)", async () => {
    const c = clientWithSelect({ data: null, error: null })
    const r = await upsertSummary(FIXTURE_SUMMARY, () => c as never)
    expect(r.ok).toBe(true)
    expect(c.from).toHaveBeenCalledWith("carfax_cache")
    expect(c.upsert).toHaveBeenCalledWith(
      expect.objectContaining({
        vin: FIXTURE_SUMMARY.vin,
        has_report: true,
        result_code: 1,
        fetched_at: FIXTURE_SUMMARY.fetchedAt,
      }),
      { onConflict: "vin" },
    )
  })
  ...
  it("catches thrown exceptions into err", async () => {
    ...
    const r = await upsertSummary(FIXTURE_SUMMARY, factory as never)
    expect(r.ok).toBe(false)
    if (!r.ok) expect(r.error.kind).toBe("exception")
  })
})
...
describe("isCacheFresh", () => {
  ...
  it("returns true within the default 24h TTL", () => {
    expect(isCacheFresh({ ...FIXTURE_SUMMARY, fetchedAt: "2026-05-01T13:00:00.000Z" }, now)).toBe(true)
  })

  it("returns false past the 24h TTL", () => {
    expect(isCacheFresh({ ...FIXTURE_SUMMARY, fetchedAt: "2026-04-30T11:00:00.000Z" }, now)).toBe(false)
  })

  it("respects a custom TTL", () => {
    const oneHourAgo = { ...FIXTURE_SUMMARY, fetchedAt: "2026-05-02T11:00:00.000Z" }
  ...
  it("returns false on unparseable fetchedAt (defensive)", () => {
    expect(isCacheFresh({ ...FIXTURE_SUMMARY, fetchedAt: "not-a-date" }, now)).toBe(false)
  })
})
