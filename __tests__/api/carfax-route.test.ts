...
const ENV = {
  ...
  CARFAX_CLIENT_ID: "id",
  CARFAX_CLIENT_SECRET: "this-is-long-enough-secret-1234",
...
const FRESH_SUMMARY = {
  ...
  // Fresh — within 24h of "now" inside the test
...
beforeEach(() => {
  ...
  readCarfaxEnvMock.mockReturnValue(ENV)
})
...
describe("GET /api/v1/vehicles/[vin]/carfax", () => {
  ...
  it("serves the cache hit on a fresh row (no live fetch)", async () => {
    ...
    expect(body.summary.vin).toBe(VALID_VIN)
    expect(fetchBadgesMock).not.toHaveBeenCalled()
  })
