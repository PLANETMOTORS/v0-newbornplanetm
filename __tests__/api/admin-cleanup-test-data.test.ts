...
vi.mock("@/lib/supabase/server", () => ({
  createClient: vi.fn(async () => ({
    auth: {
      getUser: async () => ({
        data: {
          user: currentUserEmail ? { email: currentUserEmail } : null,
        },
      }),
    },
  })),
...
function makeRequest(body: unknown): NextRequest {
  ...
  return new NextRequest("http://localhost/api/v1/admin/cleanup/test-data", init)
}
...
function resetDb() {
  ...
  fakeDb.reservations = [
    {
      id: "res-1",
      customer_name: "Devin Test",
      customer_email: "devin.test@planetmotors.ca",
    },
    {
      id: "res-2",
      customer_name: "Real Person",
      customer_email: "real@hotmail.com",
    },
  ]
...
describe("POST /api/v1/admin/cleanup/test-data — auth", () => {
  ...
  it("rejects with 401 when user is not in ADMIN_EMAILS", async () => {
    currentUserEmail = "stranger@example.com"
    const res = await POST(makeRequest({ mode: "test-pattern", dryRun: true }))
    expect(res.status).toBe(401)
  })

  it("allows access for any address listed in ADMIN_EMAILS", async () => {
...
describe("POST /api/v1/admin/cleanup/test-data — body validation", () => {
  ...
  it("rejects 400 when by-id table is invalid", async () => {
    ...
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/leads\|reservations\|trade_in_quotes/)
  })
  ...
  it("rejects 400 when by-id ids exceeds 100", async () => {
    const ids = Array.from({ length: 101 }, (_, i) => `id-${i}`)
    const res = await POST(makeRequest({ mode: "by-id", table: "leads", ids }))
    expect(res.status).toBe(400)
    const body = await res.json()
    expect(body.error).toMatch(/Max 100/)
  })
...
describe("POST /api/v1/admin/cleanup/test-data — by-id mode", () => {
  ...
  it("returns 500 with error message when DB delete fails", async () => {
  ...
  it("does not affect rows in other tables", async () => {
    await POST(
      makeRequest({ mode: "by-id", table: "leads", ids: ["lead-1"] }),
    )
  ...
  it("targets reservations table independently", async () => {
    const res = await POST(
  ...
  it("targets trade_in_quotes table independently", async () => {
    const res = await POST(
...
// ── test-pattern mode (dry run) ───────────────────────────────────

describe("POST /api/v1/admin/cleanup/test-data — test-pattern dryRun", () => {
  it("defaults to dryRun=true when dryRun is omitted", async () => {
    ...
    expect(fakeDb.leads.length).toBe(before.leads)
    expect(fakeDb.reservations.length).toBe(before.reservations)
    expect(fakeDb.trade_in_quotes.length).toBe(before.trade_in_quotes)
  })

  it("returns matched rows + summary when dryRun=true", async () => {
    const res = await POST(
    ...
    expect(Array.isArray(body.matches.leads)).toBe(true)
  ...
  it("dryRun does not call delete on any table", async () => {
...
// ── test-pattern mode (real delete) ───────────────────────────────

describe("POST /api/v1/admin/cleanup/test-data — test-pattern destructive", () => {
  ...
  it("does not include errors key in response when no failures", async () => {
    const res = await POST(
      makeRequest({ mode: "test-pattern", dryRun: false }),
    )
    const body = await res.json()
  ...
  it("returns ok:false + errors[] when one table delete fails", async () => {
  ...
  it("survives a select error on one table (returns empty matches for it)", async () => {
    ...
    expect(body.matches.leads.length).toBeGreaterThanOrEqual(0)
  })

  it("skips empty id lists gracefully (no delete call)", async () => {
    fakeDb.leads = []
    fakeDb.reservations = []
    fakeDb.trade_in_quotes = []
    const res = await POST(
