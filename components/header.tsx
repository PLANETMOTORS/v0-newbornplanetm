      ...
      <header className={`bg-background/95 backdrop-blur-md border-b border-border ${scrolled ? "shadow-sm" : ""}`} role="banner">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 py-3 lg:px-8" aria-label="Main navigation">
          ...
          <div className="flex items-center gap-2">
            {/* Sign In Button with Hamburger Icon - Premium Ellipse Style */}
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setSignInPanelOpen(true)}
              className="hidden lg:flex items-center gap-3 font-medium px-5 py-1.5 rounded-full border-2 border-[#1e3a8a] hover:bg-[#1e3a8a]/5 hover:shadow-lg transition-all"
            >
              <Menu className="w-5 h-5 text-[#1e3a8a]" />
              <div className="w-11 h-11 rounded-full bg-[#1e3a8a] flex items-center justify-center">
